...
contract Strategy is BaseStrategy, ICallee {
    ...
    function _leverUpStep(uint256 amount) internal returns (uint256) {
        ...
        (uint256 deposits, uint256 borrows, ) = getCurrentPosition();
        uint256 canBorrow =
            deposits.mul(targetCollatRatio).div(MAX_BPS).sub(borrows);
        if (canBorrow < amount) {
            amount = canBorrow;
        }
    ...
    {
        borrows = balanceOfAToken();
        deposits = balanceOfDebtToken();
        if (deposits > 0) {
    ...
    function tokenToWant(address token, uint256 amount) internal view returns (uint256) {
        ...
        uint256[] memory amounts =
            IUni(uniswapRouter).getAmountsOut(amount, path);
